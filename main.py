...
class SinKaAndroid(App):
    def build(self):
        self.title='SinKa'
        self.pokemon={}; self.ranking={}; self.trans=lambda x:x; self.rows=[]; self.owned={}; self.images={}; self.type_images={}; self.evolutions={}; self.image_loading=set(); self.image_holders={}; self.type_holders={}; self.image_cache_dir=os.path.join(self.user_data_dir,'image_cache'); self.type_cache_dir=os.path.join(self.user_data_dir,'type_cache'); self.preferences_path=os.path.join(self.user_data_dir,'preferences.json'); self.image_total=0; self.image_done=0; self.image_success=0; os.makedirs(self.image_cache_dir,exist_ok=True); os.makedirs(self.type_cache_dir,exist_ok=True)
    ...
    def load_preferences(self):
        """Restore the user's selections after an app restart."""
        try:
            with open(self.preferences_path,'r',encoding='utf-8') as f: data=json.load(f)
            self.owned={str(name):bool(value) for name,value in (data.get('owned') or {}).items() if value}
            self.league=data.get('league') if data.get('league') in LEAGUES else self.league
            self.count=max(1,min(100,int(data.get('count',self.count))))
            self.show_shadow=bool(data.get('show_shadow',False)); self.no_xl=bool(data.get('no_xl',False))
            self.no_special=bool(data.get('no_special',False)); self.no_legendary=bool(data.get('no_legendary',False)); self.no_mythical=bool(data.get('no_mythical',False))
        except Exception:
            pass

    def save_preferences(self):
        """Atomically persist owned Pokémon and filter choices in the app's private storage."""
        try:
            data={'owned':self.owned,'league':self.league,'count':self.count,'show_shadow':self.show_shadow,'no_xl':self.no_xl,'no_special':self.no_special,'no_legendary':self.no_legendary,'no_mythical':self.no_mythical}
            temporary=self.preferences_path+'.tmp'
    ...
    def build_controls(self):
        ...
        self.count_input=TextInput(text=str(self.count),input_filter='int',multiline=False,hint_text='1-100'); self.top.add_widget(self.count_input)
        ...
        self.cb_shadow.active=self.show_shadow; self.cb_xl.active=self.no_xl; self.cb_special.active=self.no_special
        self.cb_legendary.active=self.no_legendary; self.cb_mythical.active=self.no_mythical
    ...
    def set_shadow(self,inst,val): self.show_shadow=val; self.save_preferences()
    def set_xl(self,inst,val): self.no_xl=val; self.save_preferences()
    def set_special(self,inst,val): self.no_special=val; self.save_preferences()
    def set_legendary(self,inst,val): self.no_legendary=val; self.save_preferences()
    def set_mythical(self,inst,val): self.no_mythical=val; self.save_preferences()
    ...
    def on_league(self,sp,text):
        if text!=self.league:
            self.league=text; self.save_preferences()
    ...
    def refresh(self,*_):
        ...
        self.count=self.get_count(); self.save_preferences()
    ...
    def toggle_owned_cell(self,widget,r,t):
        if widget.collide_point(*t.pos):
            r['owned']=not r['owned']; self.owned[r['name']]=r['owned']; self.save_preferences()
            ...
            return True
        return False
    def _cell_layer(self, w, bg):
        """A real FloatLayout cell; its children stay inside that exact table cell."""
        holder=FloatLayout(size_hint=(None,None),size=(dp(w),dp(62)))
        holder.add_widget(BGLabel(text='',bg=bg,size_hint=(1,1),pos_hint={'x':0,'y':0}))
        return holder

    def _put_image(self,holder,data,size=52,pos_hint=None):
        try:
            if getattr(holder,'_sinka_image_added',False): return
            image=Image(texture=CoreImage(BytesIO(data),ext='png').texture,size_hint=(None,None),size=(dp(size),dp(size)))
            image.pos_hint=pos_hint or {'center_x':.5,'center_y':.5}
            holder.add_widget(image)
            holder._sinka_image_added=True
        except Exception:
            pass

    def make_poke_image(self,name,w,bg):
        holder=self._cell_layer(w,bg)
        self.image_holders.setdefault(name,[]).append((holder,52,{'center_x':.5,'center_y':.5}))
        data=self.images.get(name)
        if data: Clock.schedule_once(lambda *_: self._put_image(holder,data,52,{'center_x':.5,'center_y':.5}),0)
        return holder

    def type_widget(self,keys,w,bg,fg):
        holder=self._cell_layer(w,bg)
        label=Label(text=' / '.join(TYPE_TR.get(k,k.title()) for k in keys),font_size=dp(10),color=self.hex_rgba(fg),halign='left',valign='middle',size_hint=(None,None),size=(dp(max(30,w-72)),dp(62)),pos=(dp(66),0),text_size=(dp(max(30,w-76)),dp(62)))
        holder.add_widget(label)
        for index,key in enumerate(keys[:2]):
            icon_holder=FloatLayout(size_hint=(None,None),size=(dp(28),dp(28)),pos=(dp(5+index*30),dp(17)))
            holder.add_widget(icon_holder)
            self.type_holders.setdefault(key,[]).append(icon_holder)
            data=self.type_images.get(key)
            if data: Clock.schedule_once(lambda _,h=icon_holder,d=data:self._put_image(h,d,28,{'center_x':.5,'center_y':.5}),0)
        return holder

    def evo_widget(self,names,w,bg,fg):
        if not names: return self.cell('Evrim yok',w,fg,bg,False,'left')
        holder=self._cell_layer(w,bg)
        x=dp(5)
        for index,name in enumerate(names):
            icon_holder=FloatLayout(size_hint=(None,None),size=(dp(42),dp(42)),pos=(x,dp(10)))
            holder.add_widget(icon_holder)
            self.image_holders.setdefault(name,[]).append((icon_holder,42,{'center_x':.5,'center_y':.5}))
            data=self.images.get(name)
            if data: Clock.schedule_once(lambda _,h=icon_holder,d=data:self._put_image(h,d,42,{'center_x':.5,'center_y':.5}),0)
            x += dp(45)
            label_width=dp(min(94,max(42, len(name)*7)))
            label=Label(text=name,font_size=dp(10),color=self.hex_rgba(fg),halign='left',valign='middle',size_hint=(None,None),size=(label_width,dp(62)),pos=(x,0),text_size=(label_width,dp(62)))
            holder.add_widget(label)
            x += label_width
            if index < len(names)-1:
                arrow=Label(text='→',font_size=dp(12),color=(.33,.37,.43,1),size_hint=(None,None),size=(dp(16),dp(62)),pos=(x,0),halign='center',valign='middle',text_size=(dp(16),dp(62)))
                holder.add_widget(arrow); x += dp(16)
        return holder
    ...
    def _type_cache_path(self,key):
        return os.path.join(self.type_cache_dir, str(TYPE_ICON_ID.get(key,key))+'.png')

    def fetch_type_img(self,key):
        cp=self._type_cache_path(key)
        try:
            if os.path.exists(cp) and os.path.getsize(cp)>100: return open(cp,'rb').read()
        except Exception: pass
        type_id=TYPE_ICON_ID.get(key)
        if not type_id: return None
        try:
            data=fetch_bytes(f'{TYPE_ICON_BASE}{type_id}.png',8)
            if data:
                try:
                    with open(cp,'wb') as f: f.write(data)
                except Exception: pass
    ...
    def load_images_async(self):
        ...
        type_keys=[]
        for r in self.rows:
            pokemon_names.append(r['name']); pokemon_names.extend(r['evo']); type_keys.extend(r['types'])
        pokemon_names=list(dict.fromkeys(pokemon_names)); type_keys=list(dict.fromkeys(type_keys))
        pending=[('pokemon',n) for n in pokemon_names if n not in self.images and n not in self.image_loading]
        pending += [('type',k) for k in type_keys if k not in self.type_images and ('type:'+k) not in self.image_loading]
        self.image_total=len(pending); self.image_done=0; self.image_success=0; self.progress.value=0
        if not pending:
            self.status.text=f'✓ {len(self.rows)} Pokémon ve görseller hazır • {self.filter_summary()}'
            return
        for kind,key in pending: self.image_loading.add(key if kind=='pokemon' else 'type:'+key)
        total=len(pending); self.status.text=f'Görseller yükleniyor... 0/{total}'
        def work():
            try:
                with ThreadPoolExecutor(max_workers=12) as pool:
                    futures={pool.submit(self.fetch_img if kind=='pokemon' else self.fetch_type_img,key):(kind,key) for kind,key in pending}
                    for future in as_completed(futures):
                        ...
                        try:
                            ...
                            if data:
                                if kind=='pokemon':
                                    self.images[key]=data; Clock.schedule_once(lambda _,n=key,d=data:self._show_loaded_image(n,d),0)
                                else:
                                    self.type_images[key]=data; Clock.schedule_once(lambda _,k=key,d=data:self._show_loaded_type(k,d),0)
                ...
                for kind,key in pending: self.image_loading.discard(key if kind=='pokemon' else 'type:'+key)
                Clock.schedule_once(lambda *_: self._finish_image_loading(total),0)
        threading.Thread(target=work,daemon=True).start()

    def _show_loaded_image(self,name,data):
        for holder,size,pos_hint in self.image_holders.get(name,[]): self._put_image(holder,data,size,pos_hint)

    def _show_loaded_type(self,key,data):
        for holder in self.type_holders.get(key,[]): self._put_image(holder,data,28,{'center_x':.5,'center_y':.5})

    def _set_image_progress(self,v,done,total):
        self.progress.value=v; self.status.text=f'Görseller yükleniyor... {done}/{total} • {self.image_success} başarılı'

    def _finish_image_loading(self,total):
        self.progress.value=100
        if self.image_success < total: self.status.text=f'✓ Görseller tamamlandı • {self.image_success}/{total} yüklendi'
        else: self.status.text=f'✓ {len(self.rows)} Pokémon ve görseller hazır • {self.filter_summary()}'
    def save_a4(self):
    ...
    def gallery_output_path(self):
        """Use the tablet's visible Pictures folder, with a safe app-folder fallback."""
        filename=f'SinKa_PvP100_{self.league.replace(" ","_")}_{datetime.now().strftime("%Y%m%d_%H%M%S")}.png'
        try:
            from android.storage import primary_external_storage_path
            folder=os.path.join(primary_external_storage_path(),'Pictures','SinKa PvP 100')
            os.makedirs(folder,exist_ok=True)
            return os.path.join(folder,filename)
        except Exception:
            folder=os.path.join(self.user_data_dir,'Pictures','SinKa PvP 100')
            os.makedirs(folder,exist_ok=True)
            return os.path.join(folder,filename)

    def scan_gallery(self,path):
        """Tell Android's media library about the new image so Gallery can show it immediately."""
        try:
            from jnius import autoclass
            activity=autoclass('org.kivy.android.PythonActivity').mActivity
            scanner=autoclass('android.media.MediaScannerConnection')
            scanner.scanFile(activity,[path],['image/png'],None)
        except Exception:
            pass
    def _save_a4_worker(self):
        try:
            ...
            out=self.gallery_output_path(); img.convert('RGB').save(out,'PNG',dpi=(180,180)); self.scan_gallery(out); Clock.schedule_once(lambda *_:setattr(self.status,'text','✓ A4 pafta Galeri > SinKa PvP 100 klasörüne kaydedildi: '+out))
