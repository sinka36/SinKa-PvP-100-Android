...
class SinKaAndroid(App):
    def build(self):
        self.title='SinKa'
        self.pokemon={}; self.ranking={}; self.trans=lambda x:x; self.rows=[]; self.owned={}; self.images={}; self.type_images={}; self.evolutions={}; self.image_loading=set()
    ...
    def on_window_size(self,*_):
        ...
        # Tablo her iki yönde de kaydırılabilir; içerik sabit genişlikte tutulur.
        if hasattr(self,'hscroll'):
            self.hscroll.do_scroll_x=True; self.hscroll.do_scroll_y=False
            self.vscroll.do_scroll_x=False; self.vscroll.do_scroll_y=True
            self.vcontent.height=self.table.height
            self.hscroll.height=self.table.height
    def build_table(self):
        ...
        self.vscroll=ScrollView(do_scroll_x=False,do_scroll_y=True,bar_width=dp(9),scroll_type=['bars','content'])
        self.vcontent=BoxLayout(orientation='vertical',size_hint=(None,None),padding=0,spacing=0)
        self.hscroll=ScrollView(do_scroll_x=True,do_scroll_y=False,bar_width=dp(9),scroll_type=['bars','content'],size_hint=(None,None))
        ...
        self.vcontent.width=self.table.width
        ...
        self.hscroll.add_widget(self.table)
        self.vcontent.add_widget(self.hscroll)
        self.vcontent.bind(minimum_height=self.vcontent.setter('height'))
        self.vscroll.add_widget(self.vcontent)
    ...
    def _resize_table_view(self):
        ...
        self.hscroll.width=self.vscroll.width
        self.hscroll.height=self.table.height
        self.vcontent.width=self.hscroll.width
        self.vcontent.height=self.table.height
    ...
    def render_rows(self, load_images=True):
        ...
        old_sx=self.hscroll.scroll_x if hasattr(self,'hscroll') else 0
    ...
    def _restore_scroll(self,sx,sy):
        if hasattr(self,'hscroll'): self.hscroll.scroll_x=max(0,min(1,sx))
        if hasattr(self,'vscroll'): self.vscroll.scroll_y=max(0,min(1,sy))
    ...
    def toggle_owned_cell(self,widget,r,t):
        if widget.collide_point(*t.pos):
            ...
            sx=self.hscroll.scroll_x; sy=self.vscroll.scroll_y
    ...
    def fetch_img(self,name):
        try:
            return fetch_bytes(SPRITE_URL.format(slug=slug(name)),15)
        except Exception:
            return None
    def load_images_async(self):
        ...
        def work():
            try:
                ...
                Clock.schedule_once(lambda *_: self.render_rows(load_images=False))
